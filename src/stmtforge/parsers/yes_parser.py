...
logger = get_logger("parsers.yes")


class YesParser(BaseParser):
    BANK_NAME = "yes"
    ...
    def parse(self, pdf_path: str | Path) -> pd.DataFrame:
        ...
        logger.info(f"Yes Bank parsing: {pdf_path.name}")
        ...
        logger.info(f"Yes Bank extracted {len(df)} transactions from {pdf_path.name}")
        return df
