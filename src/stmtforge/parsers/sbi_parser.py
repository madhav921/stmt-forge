...
logger = get_logger("parsers.sbi")


class SBIParser(BaseParser):
    BANK_NAME = "sbi"

    # SBI patterns: "DD Mon YY DESCRIPTION AMOUNT D|C" (newer) or "AMOUNT Cr|Dr" (older)
    TRANSACTION_RE = re.compile(
        r"(\d{2}\s+[A-Za-z]{3}\s+\d{2,4})\s+"
        r"(.+?)\s+"
        r"(\d[\d,]*\.\d{2})\s*"
        r"([CDcd]r?)?\s*$",   # Matches: D, Dr, C, Cr (and lowercase)
        re.IGNORECASE,
    )

    def parse(self, pdf_path: str | Path) -> pd.DataFrame:
        pdf_path = Path(pdf_path)
        logger.info(f"SBI parsing: {pdf_path.name}")
        ...
        logger.info(f"SBI extracted {len(df)} transactions from {pdf_path.name}")
        return df
        ...
        for line in text.split("\n"):
            ...
            if match:
                ...
                if date and amount and amount > 0 and len(description.strip()) > 2:
                    # SBI uses D/Dr for debit, C/Cr for credit
                    if cr_dr and cr_dr.upper().startswith("C"):
                        txn_type = "credit"
                    elif cr_dr and cr_dr.upper().startswith("D"):
                        txn_type = "debit"
                    else:
                        txn_type = detect_debit_credit(amount_str, description)
                    records.append({
                        "date": date,
